 ...
 */
export const THEME_BOOTSTRAP_SCRIPT = `(function(){try{var k="${THEME_COOKIE}";var m=document.cookie.match(new RegExp("(?:^|; )"+k+"=([^;]*)"));var t=m?decodeURIComponent(m[1]):null;if(!${LIGHT_THEME_ENABLED})t="${DEFAULT_THEME}";if(t!=="light"&&t!=="dark"){t=window.matchMedia&&window.matchMedia("(prefers-color-scheme: light)").matches?"light":"${DEFAULT_THEME}";document.cookie=k+"="+t+"; path=/; max-age=${ONE_YEAR}; SameSite=Lax";}var r=document.documentElement;r.classList.toggle("dark",t==="dark");r.style.colorScheme=t;}catch(e){}})();`;
