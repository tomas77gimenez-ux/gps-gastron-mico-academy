...
export const Route = createFileRoute("/registro")({
  ...
  head: () => {
    ...
    return {
      meta: [
        { title: t("reg.headTitle") },
        { name: "description", content: t("reg.headDesc") },
        { property: "og:title", content: t("reg.headTitle") },
        { property: "og:description", content: t("reg.headOgDesc") },
      ...
      <div className="min-h-screen pt-20 pb-12 flex items-center justify-center">
        <div className="w-full max-w-md px-4 text-center">
          <div className="rounded-xl border border-border bg-card p-8">
            ...
            <h1 className="text-2xl font-bold font-display mb-2">{t("reg.checkEmail")}</h1>
            <p className="text-muted-foreground text-sm mb-6">
              {t("reg.checkEmailDesc")} <strong className="text-foreground">{email}</strong>. 
              {t("reg.clickToActivate")}
            </p>
      ...
      <div className="w-full max-w-md px-4">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary-text text-sm font-medium mb-4">
            <UserPlus className="w-4 h-4" /> {t("reg.joinCommunity")}
          </div>
          <h1 className="text-3xl font-bold font-display">{t("reg.createAccount")}</h1>
          <p className="text-muted-foreground text-sm mt-2">{t("reg.startJourney")}</p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-6 space-y-5">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-card px-2 text-muted-foreground">{t("reg.oConEmail")}</span></div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1.5">{t("reg.name")}</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={t("reg.namePlaceholder")} required
                />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">{t("reg.email")}</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5">{t("reg.password")}</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={t("reg.passwordPlaceholder")} required minLength={8}
                />
            ...
            <Button type="submit" className="w-full" disabled={loading}>
              {loading ? t("reg.registering") : t("reg.createAccount")}
            </Button>
          </form>

          <p className="text-center text-sm text-muted-foreground">
            {t("reg.alreadyHaveAccount")}
            <Link to="/login" className="text-primary-text font-medium hover:underline">{t("reg.loginLink")}</Link>
          </p>
        </div>
      </div>
    </div>
